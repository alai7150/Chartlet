...
namespace FanG
{
    ...
    public partial class Chartlet : UserControl
    {
        ...
        public void InitializeData(ArrayList[] ChartData, ArrayList XLabel, ArrayList ColorGuider)
        {
            ...
            int ChartTop = 60;
            int ChartBottom = 40;
            int ChartH = ImageH - ChartTop - ChartBottom - this.InflateBottom - this.InflateTop;
        ...
        private void PaintBackground(Graphics g, int ChartLeft, int ChartTop, int ChartW, int ChartH)
        {
            ...
            if (this.ChartType != ChartTypes.HBar)
            {
                ...
                if (this.SpecLine.Show)
                {
                    ...
                    g.DrawLine(SpenLinePen, ChartLeft - 3, (float)(ChartTop + ChartH - ChartH * this.SpecLine.LowLimit / (MaxData - MinData)), ChartLeft + ChartW, (float)(ChartTop + ChartH - ChartH * this.SpecLine.LowLimit / (MaxData - MinData)));
                    g.DrawLine(SpenLinePen, ChartLeft - 3, (float)(ChartTop + ChartH - ChartH * this.SpecLine.HighLimit / (MaxData - MinData)), ChartLeft + ChartW, (float)(ChartTop + ChartH - ChartH * this.SpecLine.HighLimit / (MaxData - MinData)));
                }
            }
            //绘制图表标题
                    ...
                    for (int i = 0; i < ElementCount; i++)
                    {
                        ...
                        for (int j = 0; j < GroupSize; j++)
                        {
                            if (RawDatas[j][i] != -0.830213m)
                            {
                                ...
                                if (this.Tips.Show)
                                {
                                    DrawTips(g, RawDatas[j][i].ToString(), BarLeft, BarTop, BarW, BarH, BelowX);
                                }
                                //else{
                    ...
                    for (int i = 0; i < ElementCount; i++)
                    {
                        ...
                        for (int j = 0; j < GroupSize; j++)
                        {
                            if (RawDatas[j][i] != -0.830213m)
                            {
                                ...
                                if (this.Tips.Show)
                                {
                                    DrawTips(g, RawDatas[j][i].ToString(), BarLeft - DP3, BarTop + DP3, BarW, BarH, false);
                                }
                                //else{
                                /******* 构建客户端参数 ********/
                                if (StrBarL == "")
                    ...
                    for (int j = 0; j < GroupSize; j++)
                    {
                        ...
                        for (i = 0; i < ElementCount - 1; i++)
                        {
                            ...
                            if (j == 0)
                            {
                                ...
                                if (this.XLabels.Show && (i % this.XLabels.SampleSize == 0))
                                {
                                    DrawXText(g, BackPen, AxisX[i], DP3, ChartTop, ChartH, PointLeft, XGrid);
                                }
                                //PointLeft调整到下一个点
                            }
                            //绘制折线
                            if (RawDatas[j][i] != -0.830213m)
                            {
                                ...
                                if (this.Dimension == ChartDimensions.Chart2D)
                                {
                                    if (RawDatas[j][i + 1] != -0.830213m)
                                        DrawLine2D(g, LinePen, PointLeft, ChartTop + ChartH - IntDatas[j][i], PointLeft + XGrid, ChartTop + ChartH - IntDatas[j][i + 1]);
                                }
                                else
                                {
                                    ...
                                    if (RawDatas[j][i + 1] != -0.830213m)
                                    {
                                        DrawLineSurface3D(g, LinePen, LineBrush, Depth, halfRadius, PointLeft, ChartTop + ChartH - IntDatas[j][i], PointLeft + XGrid, ChartTop + ChartH - IntDatas[j][i + 1]);
                                    }

                                    //绘制值标签
                                    if (this.Tips.Show && RawDatas[j][i] != -0.830213m)
                                    {
                                        DrawTips(g, RawDatas[j][i].ToString(), (int)(PointLeft - XGrid / 2), (int)(ChartTop + ChartH - IntDatas[j][i] - this.LineConnectionRadius / 2), XGrid, (int)IntDatas[j][i], false);
                                    }
                                    /******* 构建客户端参数 ********/
                        ...
                        if (this.Dimension == ChartDimensions.Chart2D)
                        {
                            ...
                            for (int k = 0; k < ElementCount; k++)
                            {
                                if (RawDatas[j][k] != -0.830213m)
                                {
                                    ...
                                    if (this.Tips.Show)
                                    {
                                        DrawTips(g, RawDatas[j][k].ToString(), (int)(PointLeft - XGrid / 2), (int)(ChartTop + ChartH - IntDatas[j][k] - this.LineConnectionRadius / 2), XGrid, (int)IntDatas[j][i], false);
                                    }
                                    /******* 构建客户端参数 ********/
                    ...
                    for (int i = 0; i < ElementCount; i++)
                    {
                        ...
                        for (int j = 0; j < GroupSize; j++)
                        {
                            if (RawDatas[j][i] != -0.830213m)
                            {
                                ...
                                if (this.Tips.Show)
                                {
                                    DrawTips(g, RawDatas[j][i].ToString(), BarLeft, BarTop + 15, BarW, BarH, BelowX);
                                }
                                //else{
                    ...
                    for (int j = 0; j < GroupSize; j++)
                    {
                        ...
                        for (i = 0; i < ElementCount; i++)
                        {
                            ...
                            if (RawDatas[j][i] != -0.830213m && XTimes[i] <= this.Trend.End)
                            {
                                ...
                                if (this.Tips.Show)
                                {
                                    DrawTips(g, RawDatas[j][i].ToString(), (int)(PointLeft), (int)(ChartTop + ChartH - IntDatas[j][i] - this.LineConnectionRadius / 2), PointLeft - LastPointLeft, (int)IntDatas[j][i], false);
                                }
                                /******* 构建客户端参数 ********/
                                WriteSrcipt(PointLeft - Depth, ChartTop + Depth + ChartH - IntDatas[j][i], RawDatas[j][i]);
                            }
